		...
		for i in self.transforms:
			if type(i) in (tuple, list):
				...
				if imgtrans is not None:
					logger.debug(f'call image transform {imgtrans.__class__.__name__} on {type(img)}')
					img = imgtrans(img)
...
class RandomNoise(Transform):
	...
	def transform(self, image):
		...
		mean = [maxval * self.mean * (random.random() - 0.5) * 2 for _ in range(3)]
...
class ToFloat(Transform):
	...
	def transform(self, img):
		if img.dtype == np.float32 or img.dtype == np.float64 or img.dtype == np.float:
			return img
